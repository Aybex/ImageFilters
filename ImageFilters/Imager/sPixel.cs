...
public struct sPixel : ICloneable, ISerializable
{
    ...
    private static readonly cRGBCache _CACHE_Y = new();
    private static readonly cRGBCache _CACHE_U = new();
    private static readonly cRGBCache _CACHE_V = new();

    private static readonly cRGBCache _CACHE_ALTERNATE_U = new();
    private static readonly cRGBCache _CACHE_ALTERNATE_V = new();

    private static readonly cRGBCache _CACHE_BRIGHTNESS = new();
    private static readonly cRGBCache _CACHE_HUE = new();
    ...
    public byte Minimum => _Min(Red, Green, Blue);

    /// <summary>
    /// Gets the maximum value of Red, Green and Blue.
    /// </summary>
    /// <value>The maximum.</value>
    public byte Maximum => _Max(Red, Green, Blue);
    ...
    public sPixel ExtractColors
    {
        get
        {
			...
			*/
            ...
            return new sPixel((byte)baseR, (byte)baseG, (byte)baseB, Alpha);
        }
    }
    ...
    public sPixel ExtractDeltas
    {
        get
        {
            ...
            var green = Green;
            var blue = Blue;
            var color = ExtractColors;
            return
              new sPixel((byte)(color.Red - red), (byte)(color.Green - green), (byte)(color.Blue - blue), Alpha);
        }
    }
    ...
    public byte Luminance => _CACHE_Y.GetOrAdd(_argbBytes & _RGB_MASK, _CalculateLuminance);
    ...
    public byte ChrominanceU => _CACHE_U.GetOrAdd(_argbBytes & _RGB_MASK, _CalculateChrominanceU);
    ...
    public byte ChrominanceV => _CACHE_V.GetOrAdd(_argbBytes & _RGB_MASK, _CalculateChrominanceV);
    ...
    public byte Alpha => _GetAlpha(_argbBytes);

    /// <summary>
    /// Gets or sets the red component.
    /// </summary>
    /// <value>The red-value.</value>
    public byte Red => _GetRed(_argbBytes);

    /// <summary>
    /// Gets or sets the green component.
    /// </summary>
    /// <value>The green-value.</value>
    public byte Green => _GetGreen(_argbBytes);

    /// <summary>
    /// Gets or sets the blue component.
    /// </summary>
    /// <value>The blue-value.</value>
    public byte Blue => _GetBlue(_argbBytes);

    #endregion

    #region float values

    public double DoubleRed => _Byte2Double(Red);
    public float SingleRed => _Byte2Single(Red);
    public double DoubleGreen => _Byte2Double(Green);
    public float SingleGreen => _Byte2Single(Green);
    public double DoubleBlue => _Byte2Double(Blue);
    public float SingleBlue => _Byte2Single(Blue);
    public double DoubleAlpha => _Byte2Double(Alpha);
    public float SingleAlpha => _Byte2Single(Alpha);
    ...
    /// <returns></returns>
    public static sPixel FromRGBA(byte red, byte green, byte blue, byte alpha = byte.MaxValue) => new(red, green, blue, alpha);
    ...
    /// <param name="alpha">The alpha value.</param>
    /// <returns></returns>
    public static sPixel FromGrey(byte grey, byte alpha = byte.MaxValue) => new(grey, alpha);
    ...
    {
        _argbBytes = (uint)alpha << _ALPHA_SHIFT | (uint)red << _RED_SHIFT | (uint)green << _GREEN_SHIFT | (uint)blue << _BLUE_SHIFT;
    }
    ...
    /// </returns>
    public override string ToString() => $"({_argbBytes:X8}) Red:{Red}, Green:{Green}, Blue:{Blue}, Alpha:{Alpha}";
    ...
    /// </returns>
    public override int GetHashCode() => (int)_argbBytes;
    ...
    /// </returns>
    public override bool Equals(object o) => o is sPixel && ((sPixel)o)._argbBytes == _argbBytes;
    ...
    /// </returns>
    public bool Equals(sPixel pixel) => pixel._argbBytes == _argbBytes;
    ...
    public bool IsLike(sPixel pixel)
    {
        ...
        var delta = Luminance - pixel.Luminance;
        if (Math.Abs(delta) > _LUMINANCE_TRIGGER)
            return false;

        delta = ChrominanceV - pixel.ChrominanceV;
        if (Math.Abs(delta) > _CHROMA_V_TRIGGER)
            return false;

        delta = ChrominanceU - pixel.ChrominanceU;
    ...
    /// </returns>
    public bool IsNotLike(sPixel pixel) => !IsLike(pixel);
    ...
    {
        return _LUMINANCE_TRIGGER * (uint)Math.Abs(Luminance - pixel.Luminance)
               + _CHROMA_V_TRIGGER * (uint)Math.Abs(ChrominanceV - pixel.ChrominanceV)
               + _CHROMA_U_TRIGGER * (uint)Math.Abs(ChrominanceU - pixel.ChrominanceU);
    }
    ...
    public void GetObjectData(SerializationInfo serializationInfo, StreamingContext _)
      => serializationInfo.AddValue("value", _argbBytes)
    ;
