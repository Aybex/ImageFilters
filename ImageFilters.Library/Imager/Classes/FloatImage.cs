...
namespace ImageFilters.Library.Imager.Classes; 
...
internal class FloatImage {
  ...
  private FloatImage(int width, int height, OutOfBoundsMode horizontalOutOfBoundsMode, OutOfBoundsMode verticalOutOfBoundsMode) {
    _width = width;
    _height = height;
    HorizontalOutOfBoundsMode = horizontalOutOfBoundsMode;
    VerticalOutOfBoundsMode = verticalOutOfBoundsMode;

    // allocate space
    var totalElements = width * height;
    _redPlane = new float[totalElements];
    _greenPlane = new float[totalElements];
    _bluePlane = new float[totalElements];
    _alphaPlane = new float[totalElements];
  }
  #endregion
  ...
  public float Red(int x, int y) {
    return _GetValueFromPlane(_redPlane, x, y, _width, _height, _horizontalOutOfBoundsHandler, _verticalOutOfBoundsHandler);
  }

  public float Green(int x, int y) {
    return _GetValueFromPlane(_greenPlane, x, y, _width, _height, _horizontalOutOfBoundsHandler, _verticalOutOfBoundsHandler);
  }

  public float Blue(int x, int y) {
    return _GetValueFromPlane(_bluePlane, x, y, _width, _height, _horizontalOutOfBoundsHandler, _verticalOutOfBoundsHandler);
  }

  public float Alpha(int x, int y) {
    return _GetValueFromPlane(_alphaPlane, x, y, _width, _height, _horizontalOutOfBoundsHandler, _verticalOutOfBoundsHandler);
  }
  #endregion
  ...
  public FloatImage Resize(int destWidth, int destHeight, KernelType method, bool centeredGrid) {
    return _Resize(destWidth, destHeight, Kernels.KERNELS[method], centeredGrid);
  }
  ...
  public FloatImage Resize(int destWidth, int destHeight, WindowType method, float radius, bool centeredGrid) {
    return _Resize(destWidth, destHeight, Windows.WINDOWS[method].WithRadius(radius), centeredGrid);
  }
  ...
  private FloatImage _Resize(int destWidth, int destHeight, Kernels.FixedRadiusKernelInfo interpolationInfo, bool centeredGrid) {
    var xScale = (float)destWidth / Width;
    var yScale = (float)destHeight / Height;
    var xStep = 1 / xScale;
    var yStep = 1 / yScale;
    float xStart, yStart;
    if (centeredGrid) {
    ...
    var result = new FloatImage(destWidth, destHeight, _horizontalOutOfBoundsMode, _verticalOutOfBoundsMode);

    // prefilter image if necessary
    if (interpolationInfo.PrefilterAlpha != null && interpolationInfo.PrefilterAlpha.Length > 0)
      _PrefilterImage(interpolationInfo.PrefilterAlpha, interpolationInfo.PrefilterScale);
    ...
    _LinScale2D(
      ...
      OutOfBoundsUtils.GetHandlerOrCrash(HorizontalOutOfBoundsMode),
      OutOfBoundsUtils.GetHandlerOrCrash(VerticalOutOfBoundsMode)
    );
    return result;
  }
    ...
    foreach (var plane in new[] {
      ...
      for (var x = 0; x < Width; x++)
        foreach (var t in alpha)
          _PrefilterScan(plane, x, Width, Height, t, _verticalOutOfBoundsMode);

      for (var y = 0; y < Height; y++)
        foreach (var t in alpha)
          _PrefilterScan(plane, +Width * y, 1, Width, t, _horizontalOutOfBoundsMode);
  ...
  private void _LinScale2D(FloatImage destination, float xStart, float xStep, float yStart, float yStep, Kernels.FixedRadiusKernelMethod kernel, float kernelRadius, bool kernelNormalize, OutOfBoundsUtils.OutOfBoundsHandler horizontalOutOfBoundsHandler, OutOfBoundsUtils.OutOfBoundsHandler verticalOutOfBoundsHandler) {
    ...
    foreach (var plane in new[] {
               Tuple.Create(_redPlane,destination._redPlane),
               Tuple.Create(_greenPlane,destination._greenPlane),
               Tuple.Create(_bluePlane,destination._bluePlane),
               Tuple.Create(_alphaPlane,destination._alphaPlane),
             }) {
